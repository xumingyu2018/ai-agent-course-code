...
// ChatPromptTemplate 参数是一个二维数组，数组第一个元素是 message 类型(system、human、ai、tool)，第二个元素是 message 内容。
...
// chatPrompt.formatMessages 方法会将模板中的占位符替换为实际的参数值，并返回一个消息数组，消息数组可以直接传给模型进行调用。
